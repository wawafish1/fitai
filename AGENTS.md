...
- Keep the user's existing model. Upload images up to 2048px at JPEG quality 0.9 and request high image detail. Recognize meal photos and nutrition labels; return individual foods with editable quantities and nutrition bases, and calculate totals deterministically. Common cooking/oil estimates are sufficient for meal images. Label values must come from the label, not guesses: distinguish per100g/per100ml/per-serving, convert kJ to kcal, and require missing intake quantities/macros to be filled. Save item breakdowns alongside existing meal totals without changing legacy records.
...
- Today lists every meal recorded today; each opens a read-only photo, full food description, and macro detail sheet. Saved meals must not expose edit or delete controls. AI food confirmation uses a wrapping, auto-growing multiline field with native phone keyboard behavior.
- On native mobile browsers, cap app bottom sheets to the actual available viewport with safe-area spacing instead of the desktop preview device height. Keep the sheet header uncompressed and its close/cancel row sticky and tappable while scrolling.
- Meal detail and meal photo/gallery confirmation sheets close by dragging down or tapping the backdrop, so do not show a duplicate top-right close/cancel action. Keep the read-only meal detail's bottom close button.
- Daily state entries in Trends can be edited or deleted after a confirmation. Editing preserves the entry's original date and stage; deleting targets that date/stage pair only. Recompute derived trends and current targets without rewriting completed review snapshots.
